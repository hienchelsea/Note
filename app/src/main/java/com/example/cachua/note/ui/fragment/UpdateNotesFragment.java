...
public class UpdateNotesFragment extends BaseFragment implements View.OnLongClickListener {
    ...
    ArrayList<RecModel> recModelArrayList = new ArrayList<>();
    ...
    protected int getLayoutResource() {
        return (R.layout.fragment_update_notes);
    }
    ...
    protected void initData(Bundle savedInstanceState) {
        ...
        // imgModelArrayList= new ArrayList<>();

        noteModels = myDataBaseHelper.getAllNote(SharedPreferencesUtils.getInt(mContext, Constants.CHECK_ID_NOTE));
        NoteModel noteModel = new NoteModel();
        noteModel = noteModels.get(0);
        idNote = noteModels.get(0).getId();
        checkTick = noteModels.get(0).getNoteNox();
        checkImg = noteModels.get(0).getNoteIMG();
        checkRec = noteModels.get(0).getNoteRec();
        checkAlarm=noteModels.get(0).getAlarm();
        checkClips=noteModels.get(0).getClips();
        checkTag=noteModels.get(0).getTag();
        colorSelect=noteModels.get(0).getColor();
        Log.i("addTableImg", checkClips+" "+checkAlarm+" "+checkTag+"onClick: ");

        edtTitle.setText(noteModel.getName());
        edtNote.setText(noteModel.getDescribe());
        ...
        if(checkTag==1){
            hagsTagModels= myDataBaseHelper.getAllHasTag(idNote);
            imvHagsTag.setBackgroundResource(R.color.colorPrimaryDark);
            Log.i("addTableImg", hagsTagModels.size()+"");
        }
        else{
            imvHagsTag.setBackgroundResource(R.color.colorPrimary);
        }
        ...
        if (checkTick == 1) {
            ...
            tickBoxModelArrayList = myDataBaseHelper.getAllBox(SharedPreferencesUtils.getInt(mContext, Constants.CHECK_ID_NOTE));
            sizeCheckTickBox= tickBoxModelArrayList.size();
            Log.i("checkTick", tickBoxModelArrayList.size() + "");
        }
        if (checkImg == 1) {
            rcIMG.setVisibility(View.VISIBLE);
            imgModelArrayList = myDataBaseHelper.getAllImg(SharedPreferencesUtils.getInt(mContext, Constants.CHECK_ID_NOTE));
            sizeCheckImg = imgModelArrayList.size();

        }
        if(checkRec==1){
            llRecording.setVisibility(View.VISIBLE);
            recModelArrayList= myDataBaseHelper.getAllRec(SharedPreferencesUtils.getInt(mContext, Constants.CHECK_ID_NOTE));
            Log.i("recModelArrayList", SharedPreferencesUtils.getInt(mContext, Constants.CHECK_ID_NOTE)+"");
            recModel= recModelArrayList.get(0);
        }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.tvSave: {
                getMethodManager().hideSoftInputFromWindow(view.getWindowToken(), 0);
                if (checkTick == 1) {
                    myDataBaseHelper.deleteTableBoxIdNote(idNote);
                    if(tickBoxModelArrayList.size()==0){
                        checkTick=0;
                    }
                    else{
                ...
                if (checkImg == 1) {
                    int k=noteModels.get(0).getId();
                    myDataBaseHelper.deleteTableImgIdNote(idNote);
                    for (int i = 0; i < imgModelArrayList.size(); i++) {
                        ImgModel imgModel = imgModelArrayList.get(i);
                        imgModel.setIdNote(idNote);
                ...
                NoteModel noteModel = new NoteModel(noteModels.get(0).getId(),
                        edtTitle.getText().toString(), edtNote.getText().toString(),
            ...
            public boolean onMenuItemSelected(MenuBuilder menu, MenuItem item) {
                switch (item.getItemId()) {
                    case R.id.menuList:
                        ...
                        llCheck.setVisibility(View.VISIBLE);
                        if (checkTickBox == 0) {
                            tickBoxModelArrayList.add(new TickBoxModel(SharedPreferencesUtils.getInt(mContext, Constants.CHECK_ID_NOTE),edtNote.getText().toString()));
                            edtNote.setText("");
                        ...
                        addDrawingFragment.setCallBack(new AddDrawingFragment.CallBack() {
                            ...
                            public void setIMG(String s) {
                                ...
                                if(SharedPreferencesUtils.getInt(mContext,Constants.CHECK_DRAW)==1){
                                    ...
                                    imgModelArrayList.add(new ImgModel(SharedPreferencesUtils.getInt(mContext, Constants.CHECK_ID_NOTE),s));
                                    Log.i("getIdNote", imgModelArrayList.size()+"setIMG: ");
                                    rcIMG.setVisibility(View.VISIBLE);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 1) {
            ...
            if (resultCode == RESULT_OK) {
                ...
                imgModelArrayList.add(new ImgModel(SharedPreferencesUtils.getInt(mContext, Constants.CHECK_ID_NOTE), path));
                Log.i("getIdNote", SharedPreferencesUtils.getInt(mContext, Constants.CHECK_ID_NOTE)+"setIMG: ");
            }
        }
        if (requestCode == 2) {
            ...
            if (resultCode == RESULT_OK) {
                ...
                ktTake = 1;
                imgModelArrayList.add(new ImgModel(SharedPreferencesUtils.getInt(mContext, Constants.CHECK_ID_NOTE), link));
            }
        }
        rcIMG.setVisibility(View.VISIBLE);
